...
def execute(filters=None):
	...
	emp_type_map = get_employment_type_map()
	...
	for ss in salary_slips:
		...
		row.append(ss.total_loan_repayment_foreign_currency)

		if currency == company_currency:
			row += [
				flt(ss.total_loan_repayment_foregin_currency) * flt(ss.exchange_rate),
				flt(ss.total_deduction) * flt(ss.exchange_rate),
				flt(ss.net_pay) * flt(ss.exchange_rate),
			]
		else:
			row += [ss.total_loan_repayment_foreign_currency, ss.total_deduction, ss.net_pay]
...
def get_conditions(filters, company_currency):
	...
	if filters.get("salary_type"):
		conditions += " and salary_type = %(salary_type)s"		
